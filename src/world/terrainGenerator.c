...
struct osn_context *gOpenSimplexNoise;

void initTerrainGenerator() {
   open_simplex_noise((U64)0xDEADBEEF, &gOpenSimplexNoise);
}

void freeTerrainGenerator() {
   open_simplex_noise_free(gOpenSimplexNoise);
   gOpenSimplexNoise = NULL;
}
...
static S32 solidCubesAroundCubeAt(S32 x, S32 y, S32 z, S32 worldX, S32 worldZ) {
   S32 solidCount = 0;
   solidCount += !isTransparentAtCube(getCubeAtWorldSpacePosition(x + worldX - 1, y, z + worldZ)) && !shouldCave(x + worldX - 1, y, z + worldZ);
   solidCount += !isTransparentAtCube(getCubeAtWorldSpacePosition(x + worldX + 1, y, z + worldZ)) && !shouldCave(x + worldX + 1, y, z + worldZ);
   solidCount += !isTransparentAtCube(getCubeAtWorldSpacePosition(x + worldX, y - 1, z + worldZ)) && !shouldCave(x + worldX, y - 1, z + worldZ);
   solidCount += !isTransparentAtCube(getCubeAtWorldSpacePosition(x + worldX, y + 1, z + worldZ)) && !shouldCave(x + worldX, y + 1, z + worldZ);
   solidCount += !isTransparentAtCube(getCubeAtWorldSpacePosition(x + worldX, y, z - 1 + worldZ)) && !shouldCave(x + worldX, y, z - 1 + worldZ);
   solidCount += !isTransparentAtCube(getCubeAtWorldSpacePosition(x + worldX, y, z + 1 + worldZ)) && !shouldCave(x + worldX, y, z + 1 + worldZ);
   return solidCount;
}
...
void generateWorld(S32 chunkX, S32 chunkZ, S32 worldX, S32 worldZ) {
   ...
   for (S32 x = 0; x < CHUNK_WIDTH; ++x) {
      for (S32 z = 0; z < CHUNK_WIDTH; ++z) {
         ...
         F64 noise = (open_simplex_noise2(gOpenSimplexNoise, (F64)(x + worldX) / stretchFactor, (F64)(z + worldZ) / stretchFactor)) * 10.0;
         for (S32 i = -5; i < 5; ++i) {
            for (S32 j = -5; j < 5; ++j) {
               noise += (open_simplex_noise2(gOpenSimplexNoise, (F64)(x + i + worldX) / (stretchFactor + i), (F64)(z + j + worldZ) / (stretchFactor + j)) * (10.0 + j)) / 2.0f;
            }
            noise /= 10.f;
         }
         ...
         getCubeAtLocalChunkPos(cubeData, x, height, z)->material = Material_Grass;
         ...
         for (S32 y = height + 1; y < MAX_CHUNK_HEIGHT; ++y) {
            // All air
            getCubeAtLocalChunkPos(cubeData, x, y, z)->material = Material_Air;
         }
         for (S32 y = 4; y < height; ++y) {
            // All dirt.
            // todo: this is where we do stuff like caves!
            // but of course in a second pass, or third pass...!
            getCubeAtLocalChunkPos(cubeData, x, y, z)->material = Material_Dirt;
         }
         for (S32 y = 0; y < 4; ++y) {
            // All bedrock
            getCubeAtLocalChunkPos(cubeData, x, y, z)->material = Material_Bedrock;
         }
      }
   }
}
...
void generateCavesAndStructures(S32 chunkX, S32 chunkZ, S32 worldX, S32 worldZ) {
   ...
   for (S32 x = 0; x < CHUNK_WIDTH; ++x) {
      for (S32 z = 0; z < CHUNK_WIDTH; ++z) {
         for (S32 y = 0; y < MAX_CHUNK_HEIGHT; ++y) {
            Cube *c = getCubeAtLocalChunkPos(cubeData, x, y, z);
   ...
   for (S32 x = 0; x < CHUNK_WIDTH; ++x) {
      for (S32 z = 0; z < CHUNK_WIDTH; ++z) {
         ...
         for (; height >= 0; --height) {
            if (getCubeAtLocalChunkPos(cubeData, x, height, z)->material != Material_Air) {
               break;
            }
         }

         if (getCubeAtLocalChunkPos(cubeData, x, height, z)->material == Material_Grass) {
            // Lets generate some trees.
            //
            // Also, a tree only has a 1/10 chance of spawning on this block.
            S32 posX = x;
            S32 posZ = z;
            if (open_simplex_noise2(gOpenSimplexNoise, (F64)posX + worldX, (F64)posZ + worldZ) >= 0.8) {
               getCubeAtLocalChunkPos(cubeData, x, height + 1, z)->material = Material_Wood_Trunk;
               getCubeAtLocalChunkPos(cubeData, x, height + 2, z)->material = Material_Wood_Trunk;
               getCubeAtLocalChunkPos(cubeData, x, height + 3, z)->material = Material_Wood_Trunk;
               for (S32 xxx = x - 3; xxx < x + 3; ++xxx) {
                  ...
                  for (S32 zzz = z - 3; zzz < z + 3; ++zzz) {
                     if (zzz >= CHUNK_WIDTH)
                        ...
                        continue;
                     getCubeAtLocalChunkPos(cubeData, xxx, height + 4, zzz)->material = Material_Leaves;
                  }
               }
            }
         }
